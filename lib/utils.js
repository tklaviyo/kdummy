...
/**
 * Resolve date range from profile property date_range_preset.
 * Returns { startDate, endDate } in YYYY-MM-DD for use with generateDate.
 * - last_3_months, last_6_months, last_12_months, last_24_months: from N months ago to today
 * - custom or unknown: returns null so caller should use date_min/date_max
 */
export function getDateRangeFromPreset(preset) {
  ...
  const months = { last_3_months: 3, last_6_months: 6, last_12_months: 12, last_24_months: 24 }
  const n = months[preset]
  if (n == null) return null
  const start = new Date(today)
  start.setMonth(today.getMonth() - n)
...
export function generateProfileProperties(customProperties = {}) {
  const defaults = {
    ...
    favorite_store: ['Store A', 'Store B', 'Store C'][Math.floor(Math.random() * 3)],
    signup_date: generateDate('2020-01-01'),
    member_since: generateDate('2020-01-01'),
    loyalty_number: 100000 + Math.floor(Math.random() * 900000),
