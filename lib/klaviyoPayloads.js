...
export function buildKlaviyoEventPayload(ev) {
  const systemKeys = new Set(['id', 'time', 'metric_name', 'eventName', 'profileId', 'profileEmail'])
  const properties = {}
  ...
  for (const [key, value] of Object.entries(ev)) {
    ...
    if (key === 'valueCurrency') {
      properties.value_currency = value
      continue
    }
    properties[key] = value
  }
  ...
  if (ev.valueCurrency != null && String(ev.valueCurrency).trim() !== '') attributes.value_currency = ev.valueCurrency
